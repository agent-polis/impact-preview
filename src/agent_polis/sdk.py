...
class AgentPolisClient:
    ...
    def get_action(self, action_id: str | UUID) -> JsonDict:
        """Get action details."""
        response = self._client.get(f"/api/v1/actions/{action_id}")
        response.raise_for_status()
        return cast(JsonDict, response.json())

    def get_preview(self, action_id: str | UUID) -> JsonDict:
        """Get impact preview for an action."""
        response = self._client.get(f"/api/v1/actions/{action_id}/preview")
        response.raise_for_status()
        return cast(JsonDict, response.json())

    def get_diff(self, action_id: str | UUID, format: str = "plain") -> JsonDict:
    ...
    def approve(self, action_id: str | UUID, comment: str | None = None) -> JsonDict:
        """Approve an action."""
    ...
    def reject(self, action_id: str | UUID, reason: str) -> JsonDict:
        """Reject an action."""
        response = self._client.post(
    ...
    def execute(self, action_id: str | UUID) -> JsonDict:
        """Mark an action as executed."""
        response = self._client.post(f"/api/v1/actions/{action_id}/execute")
        response.raise_for_status()
        return cast(JsonDict, response.json())

    def list_pending(self, all_agents: bool = False) -> JsonList:
        ...
        return cast(JsonList, payload["actions"])
