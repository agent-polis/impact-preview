...
def api_get(endpoint: str) -> JsonDict | None:
    """Make a GET request to the API."""
    try:
        response = httpx.get(f"{API_URL}{endpoint}", headers=get_headers(), timeout=30)
        return cast(JsonDict, response.json()) if response.status_code == 200 else None
    except Exception as e:
        st.error(f"API Error: {e}")
        return None


def api_post(endpoint: str, data: JsonDict) -> tuple[JsonDict | None, int]:
    ...
    try:
        ...
        return cast(JsonDict, response.json()), response.status_code
    except Exception as e:
        st.error(f"API Error: {e}")
        return None, 500
...
def show_action_card(action: JsonDict, show_actions: bool = False) -> None:
